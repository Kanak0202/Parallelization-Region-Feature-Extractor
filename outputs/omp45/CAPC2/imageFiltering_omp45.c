...
int main()
{
    ...
    #pragma capc profitability_region begin
    #pragma omp target teams distribute parallel for collapse(2) \
        map(to: image[0:N][0:N]) \
        map(from: filtered[0:N][0:N])
    for (i = 0; i < N; i++)
    {
        for (j = 0; j < N; j++)
        {
    ...
    #pragma capc profitability_region begin
    #pragma omp target teams distribute parallel for collapse(2) \
        map(to: filtered[0:N][0:N]) \
        map(from: enhanced[0:N][0:N])
    for (i = 0; i < N; i++)
    {
        for (j = 0; j < N; j++)
        {
    ...
    #pragma capc profitability_region begin
    #pragma omp target teams distribute parallel for collapse(2) \
        map(to: enhanced[0:N][0:N]) \
        map(from: output[0:N][0:N])
    for (i = 0; i < N; i++)
    {
        for (j = 0; j < N; j++)
        {
