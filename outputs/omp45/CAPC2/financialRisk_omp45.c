...
int main()
{
    ...
    #pragma capc profitability_region begin
    #pragma omp target teams distribute parallel for collapse(2) \
        map(to: assets[0:N][0:N]) \
        map(from: risk[0:N][0:N])
    for (i = 0; i < N; i++)
    {
        for (j = 0; j < N; j++)
        {
    ...
    #pragma capc profitability_region begin
    #pragma omp target teams distribute parallel for collapse(2) \
        map(to: normalized[0:N][0:N]) \
        map(from: category[0:N][0:N])
    for (i = 0; i < N; i++)
    {
        for (j = 0; j < N; j++)
        {
    ...
    #pragma capc profitability_region begin
    #pragma omp target teams distribute parallel for collapse(2) \
        map(to: normalized[0:N][0:N], category[0:N][0:N]) \
        map(from: risk[0:N][0:N])
    for (i = 0; i < N; i++)
    {
        for (j = 0; j < N; j++)
