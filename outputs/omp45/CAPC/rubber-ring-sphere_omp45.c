...
int main()
{
    ...
    while(t < MAX)
    {
        ...
        #pragma capc profitability_region begin
        #pragma omp target teams distribute parallel for map(to:XN[0:N],YN[0:N],VXN[0:N],VYN[0:N],SXXN[0:N],SXYN[0:N],SYYN[0:N],RHON[0:N],EN[0:N],XO[0:N],YO[0:N],VXO[0:N],VYO[0:N],SXXO[0:N],SXYO[0:N],SYYO[0:N],RHOO[0:N],EO[0:N]) map(from:X[0:N],Y[0:N],VX[0:N],VY[0:N],SXX[0:N],SXY[0:N],SYY[0:N],RHO[0:N],E[0:N])
        for(i = 0; i < N; i++)
        {
            X[i]   = 2.0 * XN[i] - XO[i];
            Y[i]   = 2.0 * YN[i] - YO[i];
...
void Update(double dt)
{
    ...
    #pragma capc profitability_region begin
    #pragma omp target teams distribute parallel for map(to:RHO[0:N],RHODOT[0:N],E[0:N],EDOT[0:N],X[0:N],XDOT[0:N],Y[0:N],YDOT[0:N],VX[0:N],VXDOT[0:N],VY[0:N],VYDOT[0:N],SXX[0:N],SXXDOT[0:N],SXY[0:N],SXYDOT[0:N],SYY[0:N],SYYDOT[0:N]) map(from:RHON[0:N],EN[0:N],XN[0:N],YN[0:N],VXN[0:N],VYN[0:N],SXXN[0:N],SXYN[0:N],SYYN[0:N])
    for(i = 0; i < N; i++)
    {
        RHON[i] =
            RHO[i] +
...
void initialize()
{
    ...
    */
    ...
    #pragma capc profitability_region begin
    #pragma omp target teams distribute parallel for private(angle,radius) map(from:X[0:plate_particles],Y[0:plate_particles],VX[0:plate_particles],VY[0:plate_particles],E[0:plate_particles],MASS[0:plate_particles],RHO[0:plate_particles],SXX[0:plate_particles],SXY[0:plate_particles],SYY[0:plate_particles],DAMAGE[0:plate_particles],DeltaWP[0:plate_particles])
    ...
    #pragma capc profitability_region begin
    #pragma omp target teams distribute parallel for map(to:X[0:N],Y[0:N],VX[0:N],VY[0:N],RHO[0:N],E[0:N],SXX[0:N],SXY[0:N],SYY[0:N]) map(from:XO[0:N],YO[0:N],VXO[0:N],VYO[0:N],RHOO[0:N],EO[0:N],SXXO[0:N],SXYO[0:N],SYYO[0:N],XN[0:N],YN[0:N],VXN[0:N],VYN[0:N],RHON[0:N],EN[0:N],SXXN[0:N],SXYN[0:N],SYYN[0:N])
    for(i = 0; i < N; i++)
    {
        XO[i] = X[i];
        YO[i] = Y[i];
