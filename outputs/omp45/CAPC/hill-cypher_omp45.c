// Hill Cipher - OpenMP 4.5 GPU Version
// No user/file input
...
int main()
{
	...
	#pragma omp target teams distribute parallel for map(from:mes[0:N][0:1],encrypt[0:N][0:1])
	for(i = 0; i < N; i++)
	{
		mes[i][0] = (float)(i % 26);
		encrypt[i][0] = 0.0f;
	...
	#pragma capc profitability_region begin
	#pragma omp target teams distribute parallel for collapse(2) map(to:a[0:N][0:N],mes[0:N][0:1]) map(tofrom:encrypt[0:N][0:1])
	for(i = 0; i < N; i++)
	{
		for(j = 0; j < 1; j++)
		{
