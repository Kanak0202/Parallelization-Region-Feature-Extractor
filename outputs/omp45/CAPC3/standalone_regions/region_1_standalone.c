...
int main(void)
{
    ...
    #pragma omp target enter data map(alloc:assets[0:N][0:N])
    #pragma omp taskwait
    ...
    clock_gettime(CLOCK_MONOTONIC, &__capc_t_start);
    #pragma omp target update from(assets[0:N][0:N])
    #pragma omp taskwait
    clock_gettime(CLOCK_MONOTONIC, &__capc_t_end);
    ...
    #pragma omp target exit data map(delete:assets[0:N][0:N])
    #pragma omp taskwait
