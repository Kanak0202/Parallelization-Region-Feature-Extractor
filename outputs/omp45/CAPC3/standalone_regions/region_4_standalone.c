...
int main(void)
{
    ...
    #pragma omp target enter data map(alloc:normalized[0:N][0:N], category[0:N][0:N])
    #pragma omp taskwait

    /* === Required Transfer In (Host -> Device) === */
    clock_gettime(CLOCK_MONOTONIC, &__capc_t_start);
    #pragma omp target update to(normalized[0:N][0:N])
    #pragma omp taskwait
    clock_gettime(CLOCK_MONOTONIC, &__capc_t_end);
    ...
    clock_gettime(CLOCK_MONOTONIC, &__capc_t_start);
    #pragma omp target update from(category[0:N][0:N])
    #pragma omp taskwait
    clock_gettime(CLOCK_MONOTONIC, &__capc_t_end);
    ...
    #pragma omp target exit data map(delete:normalized[0:N][0:N], category[0:N][0:N])
    #pragma omp taskwait
