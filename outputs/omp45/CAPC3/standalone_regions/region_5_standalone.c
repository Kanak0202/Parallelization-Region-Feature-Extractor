...
int main(void)
{
    ...
    /* Target Region 5; original function: main() */
    ...
    for (size_t __capc_z0_0 = 0; __capc_z0_0 < (size_t)(N); ++__capc_z0_0) {
        for (size_t __capc_z0_1 = 0; __capc_z0_1 < (size_t)(N); ++__capc_z0_1) {
            normalized[(0) + __capc_z0_0][(0) + __capc_z0_1] = 0;
        }
    }
    /* Synthetic valid input for 'category': prior CAPC producer was skipped. */
    for (size_t __capc_z1_0 = 0; __capc_z1_0 < (size_t)(N); ++__capc_z1_0) {
        for (size_t __capc_z1_1 = 0; __capc_z1_1 < (size_t)(N); ++__capc_z1_1) {
            category[(0) + __capc_z1_0][(0) + __capc_z1_1] = 0;
        }
    }
    ...
    #pragma omp target enter data map(alloc:normalized[0:N][0:N], category[0:N][0:N], risk[0:N][0:N])
    #pragma omp taskwait

    /* === Required Transfer In (Host -> Device) === */
    clock_gettime(CLOCK_MONOTONIC, &__capc_t_start);
    #pragma omp target update to(normalized[0:N][0:N], category[0:N][0:N])
    #pragma omp taskwait
    clock_gettime(CLOCK_MONOTONIC, &__capc_t_end);
    __capc_t_in = (__capc_t_end.tv_sec - __capc_t_start.tv_sec) + (__capc_t_end.tv_nsec - __capc_t_start.tv_nsec) / 1e9;

    /* === Isolated Kernel Timing for Target Region 5 === */
    clock_gettime(CLOCK_MONOTONIC, &__capc_t_start);
    ...
    printf("Region 5 Execution Breakdown:\n");
    ...
    #pragma omp target exit data map(delete:normalized[0:N][0:N], category[0:N][0:N], risk[0:N][0:N])
    #pragma omp taskwait
