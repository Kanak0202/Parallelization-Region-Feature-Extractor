...
static double __capc_rt[14]={0}; static unsigned long long __capc_rc[14]={0};
...
static void __capc_report(void){
 ...
 for(q=0;q<14;q++) if(__capc_rc[q]) printf("Region %d: total=%0.9f s, executions=%llu, average=%0.9f s\n",q,__capc_rt[q],__capc_rc[q],__capc_rt[q]/(double)__capc_rc[q]);
...
void derivatives()
{
    ...
    if(GRADCORR == 1)
    {
        ...
        #pragma omp parallel for private(i,j,xij,yij,dist,wf)
        for(i = 0; i < N-NB; i++)
        {
            for(j = 0; j < N-NB; j++)
            {
    ...
    #pragma omp parallel for private(i,j,xij,yij,vxij,vyij,dist,rhobar,drhobar,wf,visc,trace,sigxxi,sigxyi,sigyyi,sigxxj,sigxyj,sigyyj,rho2i,rho2j,mata,matb,matc,matd,wfdx,wfdy,wf_grad)
    for(i = N-NB; i < N; i++)
    {
        sigxxi = SXX[i] + P[i];
        sigyyi = SYY[i] + P[i];
...
void initialize()
{
    ...
    */
    ...
    #pragma omp parallel for private(i,angle,radius)
