...
int main(int argc, char *argv[]) {
    ...
    #pragma omp target enter data map(alloc:T[0:GRIDX+2][0:GRIDY+2],T_new[0:GRIDX+2][0:GRIDY+2])

/* ---- capc timing instrumentation: one-shot transfer calibration ---- */
{ double __t0 = omp_get_wtime();
  #pragma omp target update to(T[0:GRIDX+2])
  __capc_h2d_T = omp_get_wtime() - __t0;
  printf("[capc] H2D transfer time for 'T': %.6f s\n", __capc_h2d_T); }
{ double __t0 = omp_get_wtime();
  #pragma omp target update from(T[0:GRIDX+2])
  __capc_d2h_T = omp_get_wtime() - __t0;
  printf("[capc] D2H transfer time for 'T': %.6f s\n", __capc_d2h_T); }
{ double __t0 = omp_get_wtime();
  #pragma omp target update to(T_new[0:GRIDX+2])
  __capc_h2d_T_new = omp_get_wtime() - __t0;
  printf("[capc] H2D transfer time for 'T_new': %.6f s\n", __capc_h2d_T_new); }
{ double __t0 = omp_get_wtime();
  #pragma omp target update from(T_new[0:GRIDX+2])
  __capc_d2h_T_new = omp_get_wtime() - __t0;
...
{ double __capc_t0 = omp_get_wtime();
        #pragma omp target teams distribute parallel for collapse(2) private(i,j) map(alloc:T[0:GRIDX+2][0:GRIDY+2],T_new[0:GRIDX+2][0:GRIDY+2])
        for(i = 1; i <= GRIDX; i++) 
            for(j = 1; j <= GRIDY; j++) 
...
double __capc_t1 = omp_get_wtime(); __capc_region_0_total += (__capc_t1 - __capc_t0); __capc_region_0_count += 1; }
        ...
        #pragma capc profitability_region begin
{ double __capc_t0 = omp_get_wtime();
        #pragma omp target teams distribute parallel for collapse(2) private(i,j) reduction(max:dt) map(alloc:T[0:GRIDX+2][0:GRIDY+2],T_new[0:GRIDX+2][0:GRIDY+2])
        for(i = 1; i <= GRIDX; i++){
            for(j = 1; j <= GRIDY; j++){
...
double __capc_t1 = omp_get_wtime(); __capc_region_1_total += (__capc_t1 - __capc_t0); __capc_region_1_count += 1; }
    ...
    #pragma omp target exit data map(delete:T[0:GRIDX+2][0:GRIDY+2],T_new[0:GRIDX+2][0:GRIDY+2])
...
void init(){
    ...
    #pragma capc profitability_region begin
{ double __capc_t0 = omp_get_wtime();
    #pragma omp target teams distribute parallel for collapse(2) private(i,j) map(alloc:T[0:GRIDX+2][0:GRIDY+2])
    for(i = 0; i <= GRIDX+1; i++){
        for (j = 0; j <= GRIDY+1; j++){
            T[i][j] = 0.0;
        }
...
double __capc_t1 = omp_get_wtime(); __capc_region_2_total += (__capc_t1 - __capc_t0); __capc_region_2_count += 1; }
    ...
    #pragma capc profitability_region begin
{ double __capc_t0 = omp_get_wtime();
    #pragma omp target teams distribute parallel for private(i) map(alloc:T[0:GRIDX+2][0:GRIDY+2])
    for(i = 0; i <= GRIDX+1; i++) {
        T[i][0] = 0.0;
...
double __capc_t1 = omp_get_wtime(); __capc_region_3_total += (__capc_t1 - __capc_t0); __capc_region_3_count += 1; }
    ...
    #pragma capc profitability_region begin
{ double __capc_t0 = omp_get_wtime();
    #pragma omp target teams distribute parallel for private(j) map(alloc:T[0:GRIDX+2][0:GRIDY+2])
    for(j = 0; j <= GRIDY+1; j++) {
        T[0][j] = 0.0;
