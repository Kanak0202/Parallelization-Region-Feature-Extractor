...
static double __capc_rt[18]={0}; static unsigned long long __capc_rc[18]={0};
...
static void __capc_report(void){
 ...
 for(q=0;q<18;q++) if(__capc_rc[q]) printf("Region %d: total=%0.9f s, executions=%llu, average=%0.9f s\n",q,__capc_rt[q],__capc_rc[q],__capc_rt[q]/(double)__capc_rc[q]);
...
int main()  {
    ...
    while(t<MAX)    {
        ...
        if(loopcounter%NPRINT==0) {
            ...
            #pragma omp parallel for reduction(+:KE1,IE1,KE2,IE2,WP1)
            for(i=0; i<N; i++) {
                if(i< N-NB) {
...
void derivatives()  {
    ...
    #pragma capc profitability_region begin
    double __capc_rs_6=omp_get_wtime();
    ...
    __capc_rt[6]+=omp_get_wtime()-__capc_rs_6;
    __capc_rc[6]++;
    #pragma capc profitability_region end
    if(GRADCORR == 0)	{
        #pragma capc profitability_region begin
        double __capc_rs_7=omp_get_wtime();
        #pragma omp parallel for
    	for(i=0;i<N;i++)	{
    		wfgrad[i][0] = 1.0;
    		wfgrad[i][1] = 1.0;
        ...
        __capc_rt[7]+=omp_get_wtime()-__capc_rs_7;
        __capc_rc[7]++;
        #pragma capc profitability_region end
    }
    if(GRADCORR == 1)	{
        #pragma capc profitability_region begin
        double __capc_rs_8=omp_get_wtime();
        #pragma omp parallel for private(j,xij,yij,dist,wf)
    	for(i=0;i<N-NB;i++)	{
    		for(j=0;j<N-NB;j++)	{
				if(i!=j)	{
					xij  = X[i] - X[j];
        ...
        __capc_rt[8]+=omp_get_wtime()-__capc_rs_8;
        __capc_rc[8]++;
        #pragma capc profitability_region end
    }
    if(GRADCORR == 1)	{
        #pragma capc profitability_region begin
        double __capc_rs_9=omp_get_wtime();
        #pragma omp parallel for private(j,xij,yij,dist,wf)
    	for(i=N-NB;i<N;i++)	{
    		for(j=N-NB;j<N;j++)	{
				if(i!=j)	{
        ...
        __capc_rt[9]+=omp_get_wtime()-__capc_rs_9;
        __capc_rc[9]++;
        #pragma capc profitability_region end
    }
...
//Actual Computation
#pragma capc profitability_region begin
double __capc_rs_10=omp_get_wtime();
    for(i=0;i<N;i++)    {
        sigxxi = SXX[i] + P[i];
        sigyyi = SYY[i] + P[i];
        sigxyi = SXY[i];
    ...
    __capc_rt[10]+=omp_get_wtime()-__capc_rs_10;
    __capc_rc[10]++;
    #pragma capc profitability_region end

    //For Ball-Ball Interaction
    #pragma capc profitability_region begin
    double __capc_rs_11=omp_get_wtime();
    for(i=N-NB;i<N;i++)    {
        sigxxi = SXX[i] + P[i];
        sigyyi = SYY[i] + P[i];
        sigxyi = SXY[i];
    ...
    __capc_rt[11]+=omp_get_wtime()-__capc_rs_11;
    __capc_rc[11]++;
    #pragma capc profitability_region end
    #pragma capc profitability_region begin
    double __capc_rs_12=omp_get_wtime();
    #pragma omp parallel for
    for(i=0;i<N;i++)	{
    	if(MONA_CORR2 == 1)     {
    		VX[i]   = VX[i] + VXBAR[i];
    ...
    __capc_rt[12]+=omp_get_wtime()-__capc_rs_12;
    __capc_rc[12]++;
    #pragma capc profitability_region end
}
...
void Update(double dt)  {
    ...
    int i;
#pragma capc profitability_region begin
double __capc_rs_13=omp_get_wtime();
#pragma omp parallel for
    for(i=0;i<N;i++)   {
    ...
    __capc_rt[13]+=omp_get_wtime()-__capc_rs_13;
    __capc_rc[13]++;
    #pragma capc profitability_region end
}
...
#pragma capc profitability_region begin
double __capc_rs_14=omp_get_wtime();
#pragma omp parallel for private(shear,sigxx,sigxy,sigyy,factor,sum,diff)
    for(i=0;i<N;i++)  {
        sigxx = SXX[i] + P[i];
        sigyy = SYY[i] + P[i];
        sigxy = SXY[i];
    ...
    __capc_rt[14]+=omp_get_wtime()-__capc_rs_14;
    __capc_rc[14]++;
    #pragma capc profitability_region end
}
...
void consistency()  {
    ...
    #pragma capc profitability_region begin
    double __capc_rs_15=omp_get_wtime();
    #pragma omp parallel for private(j,xij,yij,dist,wf)
    for(i=0;i<N;i++)    {
        sumKernel[i] = 0.0;
        for(j=0;j<N;j++)    {
    ...
    __capc_rt[15]+=omp_get_wtime()-__capc_rs_15;
    __capc_rc[15]++;
    #pragma capc profitability_region end
    for(i=0;i<N;i++)    {
...
RHO[count]  = RHO0;
    ...
    #pragma capc profitability_region begin
    double __capc_rs_16=omp_get_wtime();
    #pragma omp parallel for reduction(+:tot_mass)
    for(i=0;i<N;i++) tot_mass += MASS[i];
    __capc_rt[16]+=omp_get_wtime()-__capc_rs_16;
    __capc_rc[16]++;
    #pragma capc profitability_region end
    #pragma capc profitability_region begin
    double __capc_rs_17=omp_get_wtime();
    #pragma omp parallel for reduction(+:tot_momentum)
    for(i=0;i<N;i++) tot_momentum += 0.50*MASS[i]*VX[i]*VX[i];
    __capc_rt[17]+=omp_get_wtime()-__capc_rs_17;
    __capc_rc[17]++;
    #pragma capc profitability_region end
